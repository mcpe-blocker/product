...
app.get('/user/:username/:page_id',(req,res)=>{
    ...
    `);
        con.query(`SELECT * FROM products`,(err,result)=>{
            if(err) throw err;
            var sort_num = 5;
            var to_item = page_id * sort_num - 1;
            var from_item = to_item - sort_num;
            var item_now = 0;
            res.write(`
            <div class="container shadow p-5" style="margin: 1% auto">
                <table class="table">
                <tr class="row bg-light">
                    <td class="col-2">Id</td>
                    <td class="col-4">Name</td>
                    <td class="col-2">Show</td>
                    <td class="col-2">Edit</td>
                    <td class="col-2">Delete</td>
                </tr>
            `);
            for(var i=from_item; i<=to_item; i++){
                if(result[i]!=undefined){
                    res.write(`
                        <tr class="row">
                            <td class="col-2">${result[i].id}</td>
                            <td class="col-4">${result[i].name}</td>
                            <td class="col-2"><button class="btn btn-info btn-block" type="button" onclick="window.location.pathname += '/show/${result[i].id}'"><i class="fa fa-eye"></i> Show</button></td>
                            <td class="col-2"><button class="btn btn-primary btn-block" type="button" onclick="window.location.pathname += '/edit/${result[i].id}'"><i class="fa fa-pencil"></i> Edit</button></td>
                            <td class="col-2"><button class="btn btn-danger btn-block" type="button" onclick="window.location.pathname += '/delete/${result[i].id}'"><i class="fas fa-file-excel"></i> Delete</button></td>
                        </tr>
                    `);
                }
            }
